...
public class Configure extends Command {
    ...
    public void run(Message m) throws Exception {
        ...
        switch (args[0]) {
            case "list": {
                ArrayList<String> banUsers = modulebot.voiceText.Main.settings.get(gid).get("disallowUsers");
                if (banUsers.size() == 1 && banUsers.get(0).equals("")) banUsers.remove(0);
                ArrayList<String> banRoles = modulebot.voiceText.Main.settings.get(gid).get("disallowRoles");
                if (banRoles.size() == 1 && banRoles.get(0).equals("")) banRoles.remove(0);
                ArrayList<String> admUsers = modulebot.voiceText.Main.settings.get(gid).get("alwaysUsers");
                if (admUsers.size() == 1 && admUsers.get(0).equals("")) admUsers.remove(0);
                ArrayList<String> admRoles = modulebot.voiceText.Main.settings.get(gid).get("alwaysRoles");
            ...
            case "reload": {
                ...
                while (rs.next()) {
                    ...
                    modulebot.voiceText.Main.settings.put(gid, map);
                }
                rs.close();
                st.close();
                send("Done!");
            ...
            case "add": {
                ...
                switch (args[1]) {
                    case "always": {
                        ArrayList<String> users = modulebot.voiceText.Main.settings.get(gid).get("alwaysUsers");
                        if (users.size() == 1 && users.get(0).equals("")) users.remove(0);
                        ArrayList<String> roles = modulebot.voiceText.Main.settings.get(gid).get("alwaysRoles");
                        ...
                        ps.execute();
                        ps.close();
                        modulebot.voiceText.Main.settings.get(gid).replace("alwaysUsers", users);
                        modulebot.voiceText.Main.settings.get(gid).replace("alwaysRoles", roles);
                        send("Done!");
                        break;
                    }
                    case "disallow": {
                        ...
                        ps.execute();
                        ps.close();
                        modulebot.voiceText.Main.settings.get(gid).replace("disallowUsers", users);
                        modulebot.voiceText.Main.settings.get(gid).replace("disallowRoles", roles);
                        send("Done!");
                        break;
                    }
                    default:
